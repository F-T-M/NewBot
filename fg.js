...
//---------𝗥𝗘𝗗𝗘𝗦 𝗦𝗢𝗖𝗜𝗔𝗟𝗘𝗦---------
const soportefg = 'En estos momentos están en modo privado' 
const instafg = 'En estos momentos están en modo privado'
...
linkgp = await Fg.groupInviteCode (from)
    ...
    if (budy.includes(`https://chat.whatsapp.com/Ly4I2LObSvW8VgOnJjofgE`)) return reply('✳️ Menos mal que este enlace es de este grupo')
...
Fg.groupRemove(from, [sender])
 ...
 if (!isGroup && isCmd) console.log(color('[BOSS]','aqua'), "Pv", color(command, "blue"), "de", (sender.split('@')[0]),  args.length)
if (isGroup && isCmd) console.log(color('[BOSS]','aqua'), "Gp", color(command, "green"), "de", (sender.split('@')[0]), "en", (groupName), args.length)
...
if (!isVerify) return reply(userB(prefix))
  if (isBanned) return reply(banf())
const fgnum = `6283896811444@s.whatsapp.net`
const crisnum = "6283896811444@s.whatsapp.net"
...
💡Verificación completa escribe *${prefix}Help* *para ver el Menu*`
...
if (!isVerify) return reply(userB(prefix))
  if (isBanned) return reply(banf())
  yoi = q
  if (args.length < 1) return reply('✳️ Ingrese las funciones que desee en el Bot *Boss*')
...
if (!isVerify) return reply(userB(prefix))
  if (isBanned) return reply(banf())
				...
				buff = `https://api.zeks.me/api/magernulis?apikey=WRhywqyYC0doYNeqTZymcB2SjrT&nama=Boss&kelas=6-08&text=${q}&tinta=6`
...
case 'enlinea':
 if (!isVerify) return reply(userB(prefix))
  ...
  if (!isGroup) return reply(group())
			    ...
			    Fg.sendMessage(from, 'En linea:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join`\n` + `\n*©POWERED BY Boss ┃ ᴮᴼᵀ*`, text, { quoted: mek,
...
var mantap2 = await convertSticker(bas642, `💎 Lucero`, `🔮 Boss ┃ ᴮᴼᵀ`)
...
var mantap2r = await convertSticker(bas642r, `💎 Lucero`, `🔮 Boss ┃ ᴮᴼᵀ`)
...
var mantap2c = await convertSticker(bas642c, `💎 Lucero`, `🔮 Boss ┃ ᴮᴼᵀ`)
...
anu = args.join(' ').split('|')
satu = anu[0] !== '' ? anu[0] : `🔮 Boss ┃ ᴮᴼᵀ `
dua = typeof anu[1] !== 'undefined' ? anu[1] : `💎 Lucero`
...
dua = typeof anu[1] !== 'undefined' ? anu[1] : `Boss ofc`
...
if (isGroup && budy != undefined) {
	...
	console.log(color('[CHAT]', 'red'), 'PRIVADO', color(sender.split('@')[0]))
	}		
} catch (e) {
     e = String(e) 
